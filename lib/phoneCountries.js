/** Common dial codes for the signup phone picker. Default: US. */
export const PHONE_COUNTRIES = [
  { code: 'US', name: 'United States', dial: '+1' },
  { code: 'CA', name: 'Canada', dial: '+1' },
  { code: 'GB', name: 'United Kingdom', dial: '+44' },
  { code: 'AU', name: 'Australia', dial: '+61' },
  { code: 'NZ', name: 'New Zealand', dial: '+64' },
  { code: 'IE', name: 'Ireland', dial: '+353' },
  { code: 'DE', name: 'Germany', dial: '+49' },
  { code: 'FR', name: 'France', dial: '+33' },
  { code: 'ES', name: 'Spain', dial: '+34' },
  { code: 'IT', name: 'Italy', dial: '+39' },
  { code: 'NL', name: 'Netherlands', dial: '+31' },
  { code: 'BE', name: 'Belgium', dial: '+32' },
  { code: 'CH', name: 'Switzerland', dial: '+41' },
  { code: 'AT', name: 'Austria', dial: '+43' },
  { code: 'SE', name: 'Sweden', dial: '+46' },
  { code: 'NO', name: 'Norway', dial: '+47' },
  { code: 'DK', name: 'Denmark', dial: '+45' },
  { code: 'FI', name: 'Finland', dial: '+358' },
  { code: 'PT', name: 'Portugal', dial: '+351' },
  { code: 'PL', name: 'Poland', dial: '+48' },
  { code: 'IN', name: 'India', dial: '+91' },
  { code: 'PK', name: 'Pakistan', dial: '+92' },
  { code: 'BD', name: 'Bangladesh', dial: '+880' },
  { code: 'AE', name: 'United Arab Emirates', dial: '+971' },
  { code: 'SA', name: 'Saudi Arabia', dial: '+966' },
  { code: 'SG', name: 'Singapore', dial: '+65' },
  { code: 'MY', name: 'Malaysia', dial: '+60' },
  { code: 'PH', name: 'Philippines', dial: '+63' },
  { code: 'JP', name: 'Japan', dial: '+81' },
  { code: 'KR', name: 'South Korea', dial: '+82' },
  { code: 'CN', name: 'China', dial: '+86' },
  { code: 'HK', name: 'Hong Kong', dial: '+852' },
  { code: 'TW', name: 'Taiwan', dial: '+886' },
  { code: 'BR', name: 'Brazil', dial: '+55' },
  { code: 'MX', name: 'Mexico', dial: '+52' },
  { code: 'AR', name: 'Argentina', dial: '+54' },
  { code: 'ZA', name: 'South Africa', dial: '+27' },
  { code: 'NG', name: 'Nigeria', dial: '+234' },
  { code: 'KE', name: 'Kenya', dial: '+254' },
  { code: 'EG', name: 'Egypt', dial: '+20' },
];

export const DEFAULT_PHONE_COUNTRY = 'US';

export const getPhoneCountry = (code) =>
  PHONE_COUNTRIES.find((c) => c.code === code) || PHONE_COUNTRIES[0];

/** Digits only for the national number portion. */
export const digitsOnly = (value) => String(value || '').replace(/\D/g, '');

/**
 * Build E.164-ish phone for storage when national digits are present.
 * Empty national number → empty string (optional field).
 */
export const formatPhoneForSubmit = (countryCode, nationalNumber) => {
  const digits = digitsOnly(nationalNumber);
  if (!digits) return '';
  const country = getPhoneCountry(countryCode);
  return `${country.dial}${digits}`;
};

/**
 * Split a stored phone back into picker state for editing. Longest dial code
 * wins so +1 does not shadow +1xx. Shared dial codes resolve to the first
 * country in the list, which is why the picker stays editable afterwards.
 */
export const splitStoredPhone = (stored) => {
  const raw = String(stored || '').trim();
  if (!raw.startsWith('+')) {
    return { country: DEFAULT_PHONE_COUNTRY, national: digitsOnly(raw) };
  }

  const match = [...PHONE_COUNTRIES]
    .sort((a, b) => b.dial.length - a.dial.length)
    .find((c) => raw.startsWith(c.dial));

  if (!match) return { country: DEFAULT_PHONE_COUNTRY, national: digitsOnly(raw) };
  return { country: match.code, national: digitsOnly(raw.slice(match.dial.length)) };
};
